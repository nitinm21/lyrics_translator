...
export async function geniusScrape(url: string): Promise<string> {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      if (isRetryable(res.status) && attempt < MAX_RETRIES) {
        lastError = new Error(`Genius scrape error: ${res.status} ${res.statusText}`);
        continue;
      }

      throw new Error(`Genius scrape error: ${res.status} ${res.statusText}`);
    } catch (error) {
      if (error instanceof Error && error.name === "TimeoutError") {
        lastError = new Error("Genius page request timed out");
        if (attempt < MAX_RETRIES) continue;
        throw lastError;
      }
      throw error;
  ...
  throw lastError ?? new Error("Genius scrape request failed");
}
